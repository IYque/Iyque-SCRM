...
@Service
@Slf4j
public class WeaviateVectorStore implements IYqueVectorStore{
    ...
    public WeaviateClient getClient() {
        ...
        if(StringUtils.isEmpty(paramConfig.getVector().getProtocol())
        || StringUtils.isEmpty(paramConfig.getVector().getHost())
        || StringUtils.isEmpty(paramConfig.getVector().getClassName())){
            log.error("请配置向量数据库相关参数");

            return null;
        }

        Config config = new Config(paramConfig.getVector().getProtocol(), paramConfig.getVector().getHost());
    ...
    public Result<Boolean> createSchema(String kid) {
        ...
        WeaviateClass clazz = WeaviateClass.builder()
                .className(paramConfig.getVector().getClassName() + kid)
    ...
    public void removeByKidAndFid(String kid, String fid) {
        ...
        if(null != client){
            ...
            Result<GraphQLResponse> result = client.graphQL().get()
                    .withClassName(paramConfig.getVector().getClassName() + kid)
                    .withFields(fieldId)
                    .withWhere(where)
                    .run();
            ...
            ArrayList<LinkedTreeMap> m = l.get(paramConfig.getVector().getClassName() + kid);
    ...
    public void removeByKid(String kid) {
        WeaviateClient client = getClient();
        if(null != client){
            Result<Boolean> result = client.schema().classDeleter().withClassName(paramConfig.getVector().getClassName() + kid).run();
            if (result.hasErrors()) {
